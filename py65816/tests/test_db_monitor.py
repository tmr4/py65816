...
import os
import tempfile
from py65816.db_monitor import dbMonitor as Monitor
...
class MonitorTests(unittest.TestCase):
    ...
    def test_do_add_label_overflow_error(self):
        stdout = StringIO()
        mon = Monitor(stdout=stdout)
        mon.do_add_label('$1000000 toobig')
        out = stdout.getvalue()
        err = "Overflow error: $1000000 toobig\n"
        self.assertTrue(out.startswith(err))
    ...
    def test_do_assemble_outputs_disassembly(self):
        ...
        out = stdout.getvalue()
        self.assertEqual("$00:c000  a9 ab     LDA #$ab\n", out)
    ...
    def test_do_assemble_passes_addr_for_relative_branch_calc(self):
        ...
        out = stdout.getvalue()
        self.assertEqual("$00:4000  70 03     BVS $4005\n", out)
    ...
    def test_disassemble_wraps_an_instruction_around_memory(self):
        ...
        out = stdout.getvalue()
        disasm = "$00:ffff  20 d2 ff  JSR $ffd2\n"
        self.assertEqual(out, disasm)
    ...
    def test_do_width_with_no_args_shows_current_width(self):
        ...
        mon.do_width('')
        out = stdout.getvalue()
        self.assertEqual("Terminal width is 72\n", out)
    ...
    # *** TODO: py65 fails this, at least on my PC ***
    def dont_test_argv_load(self):
        with tempfile.NamedTemporaryFile('wb+') as f:
            data = bytearray([0xab, 0xcd])
            f.write(data)
            f.flush()

            argv = ['py65816mon', '--load', f.name]
            stdout = StringIO()
            mon = Monitor(argv=argv, stdout=stdout)
            self.assertEqual(list(data), mon._mpu.memory[:len(data)])

    # *** TODO: py65 fails this, at least on my PC ***
    def dont_test_argv_rom(self):
        with tempfile.NamedTemporaryFile('wb+') as f:
            rom = bytearray(4096)
            rom[0]  = 0xea          # f000 nop
            rom[1]  = 0xea          # f001 nop
    ...
    # *** TODO: py65 fails this, at least on my PC ***
    def dont_test_argv_combination_rom_mpu(self):
        with tempfile.NamedTemporaryFile('wb+') as f:
            rom = bytearray(4096)
            rom[0]  = 0xea          # f000 nop
            rom[1]  = 0xea          # f001 nop
            ...
            argv = ['py65816mon', '--rom', f.name, '--mpu', '65c816',]
            stdout = StringIO()
            mon = Monitor(argv=argv, stdout=stdout)
            self.assertEqual('65C816', mon._mpu.name)
            self.assertEqual(list(rom), mon._mpu.memory[-len(rom):])
            self.assertEqual(0xf002, mon._mpu.pc)
